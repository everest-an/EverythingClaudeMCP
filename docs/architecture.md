...
### 2.1 LatentMAS 适配层 (`src/adapter/`)

#### 支持模型架构对比

LatentMAS 原生支持 Qwen3-4B/14B（默认），同时兼容 Qwen2.5-Coder-1.5B 作为轻量降级方案。三者关键架构差异：
...
class AdaptedModelWrapper:
    """
    基于 LatentMAS ModelWrapper 适配，支持多模型动态加载。
...
├── src/
│   ├── adapter/                        ← LatentMAS 多模型适配 (Qwen3/Qwen2.5)
│   │   ├── __init__.py
│   │   ├── config.py                   ← 模型常量
